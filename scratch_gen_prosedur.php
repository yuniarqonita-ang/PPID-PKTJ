...
$procedures = [
    [
        ...
        'subtitle' => 'SOP Standar Maklumat Pelayanan Publik.',
        'filename' => 'sop-maklumat.blade.php'
    ],
    [
        'prefix' => 'sop_biaya',
        'title' => 'Standar Biaya',
        'subtitle' => 'SOP Standar Biaya Layanan Informasi.',
        'filename' => 'sop-biaya.blade.php'
    ],
    [
        'prefix' => 'sop_waktu',
        'title' => 'Waktu Layanan',
        'subtitle' => 'SOP Standar Waktu Pelayanan Informasi.',
        'filename' => 'sop-waktu.blade.php'
    ],
    [
        'prefix' => 'sop_alur_permohonan',
        'title' => 'Alur Permohonan',
        'subtitle' => 'Prosedur Tata Cara Permohonan Informasi.',
        'filename' => 'sop-alur-permohonan.blade.php'
    ],
    [
        'prefix' => 'sop_alur_keberatan',
        'title' => 'Alur Keberatan',
        'subtitle' => 'Prosedur Tata Cara Pengajuan Keberatan.',
        'filename' => 'sop-alur-keberatan.blade.php'
    ]
];
...
<div class="space-y-8 animate-fade-in lg:px-8">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <h1 class="text-3xl md:text-5xl font-black tracking-tight leading-tight text-white mb-2">
                        SOP <span class="text-[#ffc107]">[[TITLE]]</span>
                    </h1>
                    <p class="text-blue-50 text-lg font-bold max-w-2xl opacity-90">[[SUBTITLE]]</p>
                </div>
            </div>

            <div class="flex items-center gap-4">
                <a href="{{ route('prosedur.[[PREFIX_DASH]]') }}" target="_blank" class="px-6 py-4 bg-white/10 border border-white/20 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-white/20 transition-all flex items-center">
    ...
    <form id="sop-form" action="{{ route('admin.halaman-custom.store', $prefix) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
        @csrf
            ...
            <div class="p-10 space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="judul_hero" value="{{ $settings[$prefix . '_judul_hero'] ?? 'SOP [[TITLE]]' }}"
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="tagline_hero" value="{{ $settings[$prefix . '_tagline_hero'] ?? '' }}"
                            class="w-full px-6 py-5 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 text-lg font-bold text-[#004a99]">
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div class="bg-white rounded-2xl shadow-xl border-2 border-slate-100 overflow-hidden">
                <div class="p-10 space-y-8">
                    ...
                    <div class="relative aspect-[3/4] bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center">
                        @if(isset($settings[$prefix . '_gambar_sop']))
                            <img id="preview_sop" src="{{ asset('storage/halaman/' . $settings[$prefix . '_gambar_sop']) }}" class="w-full h-full object-contain">
                        @else
            ...
            <div class="bg-white rounded-2xl shadow-xl border-2 border-slate-100 overflow-hidden">
                <div class="p-10 space-y-8">
                    ...
                    <div class="relative aspect-[3/4] bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center">
                        @if(isset($settings[$prefix . '_gambar_proses']))
                            <img id="preview_proses" src="{{ asset('storage/halaman/' . $settings[$prefix . '_gambar_proses']) }}" class="w-full h-full object-contain">
                        @else
                            <div class="text-center">
            ...
            <!-- FULL WIDTH: YOUTUBE -->
            <div class="lg:col-span-2 bg-[#004a99] rounded-3xl shadow-2xl p-10 text-white relative overflow-hidden">
                ...
                <div class="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div class="space-y-8">
                        ...
                        <div class="space-y-6">
                            <div class="space-y-3">
                                ...
                                <input type="url" name="youtube_link" value="{{ $settings[$prefix . '_youtube_link'] ?? '' }}" 
                                    class="w-full px-8 py-5 bg-white/10 border-2 border-white/20 rounded-2xl focus:bg-white focus:text-[#004a99] transition-all font-bold text-lg text-white"
                    ...
                    <div class="relative aspect-video bg-black rounded-2xl overflow-hidden shadow-inner border-4 border-white/10">
                        @php
                            $videoId = '';
                            if (!empty($settings[$prefix . '_youtube_link'])) {
                                $link = $settings[$prefix . '_youtube_link'];
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl shadow-xl border-2 border-slate-100 p-10">
                ...
                <div class="space-y-4">
                    ...
                    <textarea name="konten" class="tinymce-editor">{{ $settings[$prefix . '_konten'] ?? '' }}</textarea>
...
foreach ($procedures as $proc) {
    $content = str_replace('[[PREFIX]]', $proc['prefix'], $template);
    $content = str_replace('[[TITLE]]', $proc['title'], $content);
    $content = str_replace('[[SUBTITLE]]', $proc['subtitle'], $content);
    $content = str_replace('[[PREFIX_DASH]]', str_replace('_', '-', $proc['prefix']), $content);
    
    file_put_contents('resources/views/admin/prosedur/' . $proc['filename'], $content);
    echo "Generated: " . $proc['filename'] . "\n";
}
