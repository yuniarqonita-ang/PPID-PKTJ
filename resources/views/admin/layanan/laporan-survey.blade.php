...
<div class="min-h-screen bg-[#f8f9fa] p-4 md:p-6 text-gray-800">
    <div class="max-w-7xl mx-auto space-y-8 uppercase">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                ...
                <h1 class="text-3xl font-black text-[#004a99] uppercase tracking-tight">
                    <i class="fas fa-poll-h mr-2 text-[#ffc107]"></i> Laporan Survey Kepuasan
                </h1>
                <p class="text-gray-500 font-medium mt-1 uppercase tracking-widest text-[10px]">Pusat Kendali Evaluasi Indeks Kepuasan Masyarakat (IKM)</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ url('/laporan/survey') }}" target="_blank" class="px-6 py-3 bg-white border border-gray-200 text-[#004a99] font-bold rounded-xl shadow-sm hover:bg-gray-50 transition-all flex items-center">
        ...
        @if(session('success'))
            <div class="bg-emerald-100 border-l-4 border-emerald-500 p-4 rounded-xl shadow-sm flex items-center animate-fade-in-down mb-6">
                <div class="w-10 h-10 bg-emerald-500 rounded-full flex items-center justify-center mr-3 shadow-lg shadow-emerald-500/20">
                    <i class="fas fa-check text-white"></i>
                </div>
                <p class="text-emerald-800 font-bold text-xs">{{ session('success') }}</p>
            </div>
        @endif
        ...
        <form action="{{ route('admin.halaman-custom.store', 'laporan_survey') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl ring-1 ring-gray-200 overflow-hidden border-l-8 border-[#ffc107]">
                        <div class="p-8 space-y-6">
                            <h3 class="text-xs font-black text-[#004a99] uppercase tracking-[0.2em] flex items-center mb-6">
                                <i class="fas fa-microscope mr-3 text-[#ffc107]"></i> Metodologi & Visual Hero
                            </h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 text-gray-800">Judul Hero Laporan</label>
                                    <input type="text" name="judul_hero" value="{{ $settings['laporan_survey_judul_hero'] ?? '' }}" required
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="tagline_hero" value="{{ $settings['laporan_survey_tagline_hero'] ?? '' }}" required
                                        class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-gray-800 font-bold focus:bg-white focus:ring-4 focus:ring-[#004a99]/5 focus:border-[#004a99] outline-none transition-all uppercase placeholder-gray-200">
                                </div>
                            </div>
                            <div class="space-y-4 pt-4">
                                <label class="text-[10px] font-black text-[#004a99] uppercase tracking-widest ml-1 flex items-center">
                                    <i class="fas fa-vial mr-2 text-[#ffc107]"></i> Deskripsi Metodologi Survey
                                </label>
                                <div class="rounded-3xl overflow-hidden border border-gray-100">
                                    <textarea id="editor_metodologi" name="metodologi" class="tinymce-editor text-gray-800">{!! $settings['laporan_survey_metodologi'] ?? '' !!}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-8 space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-4">
                                <div class="space-y-2 text-gray-800">
                                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 text-gray-800">Tahun Laporan IKM</label>
                                    <div class="relative">
                                        <select name="tahun_laporan" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-xs font-black text-[#004a99] focus:bg-white focus:ring-4 focus:ring-[#004a99]/5 outline-none appearance-none cursor-pointer">
                                            @for($year = date('Y'); $year >= date('Y') - 5; $year--)
                                                <option value="{{ $year }}" {{ ($settings['laporan_survey_tahun_laporan'] ?? '') == $year ? 'selected' : '' }}>TAHUN {{ $year }}</option>
                                            @endfor
                                        </select>
                                ...
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 text-gray-800 text-gray-800">Periode Publikasi</label>
                                    <div class="relative">
                                        <select name="jenis_laporan" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-xs font-black text-[#004a99] focus:bg-white focus:ring-4 focus:ring-[#004a99]/5 outline-none appearance-none cursor-pointer">
                                            @php $jenis = $settings['laporan_survey_jenis_laporan'] ?? ''; @endphp
                                            <option value="tahunan" {{ $jenis == 'tahunan' ? 'selected' : '' }}>PUBLIKASI TAHUNAN</option>
                                            <option value="semesteran" {{ $jenis == 'semesteran' ? 'selected' : '' }}>PUBLIKASI SEMESTERAN</option>
                                        </select>
                                        <i class="fas fa-chevron-down absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"></i>
                                    </div>
                                </div>
                            ...
                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-[#004a99] uppercase tracking-widest ml-1 flex items-center">
                                    <i class="fas fa-star mr-2 text-[#ffc107]"></i> Ringkasan Eksekutif Kepuasan
                                </label>
                                <div class="rounded-3xl overflow-hidden border border-gray-100">
                                    <textarea id="editor_ringkasan" name="ringkasan_eksekutif" class="tinymce-editor text-gray-800">{!! $settings['laporan_survey_ringkasan_eksekutif'] ?? '' !!}</textarea>
                                </div>
                            </div>

                            <div class="space-y-4">
                                <label class="text-[10px] font-black text-[#004a99] uppercase tracking-widest ml-1 flex items-center text-gray-800">
                                    <i class="fas fa-chart-pie mr-2 text-[#ffc107]"></i> Analisis & Kesimpulan IKM Lengkap
                                </label>
                                <div class="rounded-3xl overflow-hidden border border-gray-100">
                                    <textarea id="editor_laporan" name="isi_laporan" class="tinymce-editor text-gray-800 text-gray-800">{!! $settings['laporan_survey_isi_laporan'] ?? '' !!}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl ring-1 ring-gray-200 p-8 space-y-6">
                        <h3 class="text-xs font-black text-[#004a99] uppercase tracking-widest border-b pb-4 flex items-center text-gray-800">
                            <i class="fas fa-file-pdf mr-2 text-emerald-500"></i> Lampiran Survey
                        </h3>
                        
                        <div class="space-y-4">
                            <div class="relative group cursor-pointer border-2 border-dashed border-gray-100 rounded-2xl p-6 hover:border-[#004a99] hover:bg-emerald-50/30 transition-all text-center" onclick="document.getElementById('file_laporan').click()">
                                <i class="fas fa-file-import text-3xl text-gray-200 mb-2 group-hover:text-[#004a99] transition-colors"></i>
                            ...
                            @if(isset($settings['laporan_survey_file_laporan']))
                            <div class="p-4 bg-emerald-600 rounded-2xl text-white flex items-center gap-4 shadow-lg shadow-emerald-500/20">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-[9px] font-bold truncate tracking-tight">HASIL_SURVEY_IKM_PKTJ.PDF</p>
                                </div>
                            </div>
                            @endif
                        </div>
                    ...
                    <!-- TIPS -->
                    <div class="bg-gradient-to-br from-[#004a99] to-emerald-800 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                        ...
                        <p class="text-[10px] leading-relaxed opacity-80 font-medium text-gray-800">
                            Data survey kepuasan masyarakat sangat vital untuk meningkatkan standar pelayanan. Pastikan narasi menyertakan tindak lanjut dari umpan balik pengguna.
                        </p>
                    </div>
                    ...
                    <div class="bg-gray-50 rounded-3xl p-6 border border-gray-200">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest">PUBLIKASI SURVEI AKTIF</span>
                        </div>
                    </div>
            ...
            <div class="bg-white/90 backdrop-blur-xl h-24 rounded-3xl shadow-2xl ring-1 ring-gray-200 px-8 flex items-center justify-between sticky bottom-6 z-50 animate-fade-in-up">
                <div class="hidden md:flex items-center gap-4">
                    <div class="w-10 h-10 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center shadow-sm">
                        <i class="fas fa-chart-line"></i>
                    </div>
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Sistem Audit Analytics Aktif</p>
                </div>
                <div class="flex gap-4 w-full md:w-auto uppercase font-bold text-gray-800">
                    ...
                    <button type="submit" class="flex-[2] md:flex-none px-12 py-3 bg-[#004a99] text-white font-black text-[10px] rounded-xl shadow-lg shadow-blue-500/20 transform hover:scale-105 transition-all tracking-widest">
                        <i class="fas fa-save mr-2 text-[#ffc107]"></i> SIMPAN DATA SURVEY
                    </button>
                </div>
            </div>
