    ...
    <div class="bg-gradient-to-br from-[#004a99] via-[#005bb5] to-[#006ccf] rounded-[2rem] p-10 md:p-12 shadow-xl text-white relative overflow-hidden mb-10">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            ...
            <div class="flex items-center gap-4">
                <a href="{{ route('informasi.setiap-saat') }}" target="_blank" class="px-6 py-4 bg-white/10 border border-white/20 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-white/20 transition-all flex items-center">
                    ...
                    <tbody class="divide-y divide-gray-100 text-xs">
                        ...
                        <tr class="hover:bg-blue-50/40 transition-colors group {{ (!$item->file_path && !$hasLinks) ? 'bg-red-50/20' : '' }}" data-link-type="{{ $linkType }}">
                            ...
                            <td class="py-3 px-3 text-center border-r border-slate-100">
                                <form action="{{ route('admin.informasi.toggle-status', ['type' => 'setiapsaat', 'id' => $item->id]) }}" method="POST" class="inline-block toggle-status-form">
                                    @csrf
                                    <button type="submit" 
                            ...
                            <td class="py-3 px-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    @endif
                                    <a href="{{ route('admin.informasi.setiapsaat.edit', $item->id) }}" class="p-1.5 bg-blue-50 text-[#004a99] rounded-lg hover:bg-[#004a99] hover:text-white transition-all shadow-xs" title="Edit Informasi">
                        ...
                        <tr>
                            <td colspan="11" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-400 text-sm mt-1">Silahkan tambahkan informasi setiap saat pertama Anda</p>
                                    <a href="{{ route('admin.informasi.setiapsaat.create') }}" class="mt-6 px-6 py-2 bg-[#004a99] text-white font-bold rounded-xl text-xs uppercase tracking-widest">Tambah Data</a>
                                </div>
                            </td>
                        </tr>
                        @endforelse
...
<script>
    ...
    function confirmDelete(id) {
        ...
        form.action = `/admin/informasi/setiap-saat/${id}`;
        modal.classList.remove('hidden');
        setTimeout(() => {
            modal.classList.add('opacity-100');
            modal.querySelector('div').classList.add('scale-100');
