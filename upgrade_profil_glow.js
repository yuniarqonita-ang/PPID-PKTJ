...
files.forEach(file => {
    ...
    if (!content.includes('card-glow') && content.includes('<div class="bg-slate-800/80 backdrop-blur-xl roun')) {
        ...
        // And remember to close the relative group div at the end (before </x-app-layout>)
        // We will do a generic replacement for the closing div right after </form> -> </div> -> </div>
    ...
    content = content.replace(/class="w-full px-4 py-2 border-2 border-slate-600 bg-slate-900\/50 text-white placeholder-slate-500 rounded-lg focus:border-cyan-500 focus:ring-cyan-500 focus:outline-none"/g, 
        'class="w-full px-4 py-3 border-2 border-slate-600/50 bg-slate-900/60 text-white placeholder-slate-500 rounded-xl focus:border-cyan-400 focus:ring-4 focus:ring-cyan-500/20 focus:outline-none transition-all duration-300 shadow-inner"');

    // 3. Upgrade basic buttons to animated glowing buttons
    content = content.replace(/class="bg-green-500 hover:bg-green-600 text-white css-placeholder-green-glow"/g, ''); // cleanup if any
