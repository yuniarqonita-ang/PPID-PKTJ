...
<div class="min-h-screen bg-[#f8f9fa] p-4 md:p-6 text-gray-800">
    <div class="max-w-7xl mx-auto space-y-8 text-gray-800">
        ...
        <div class="flex items-center justify-between text-gray-800">
            ...
            <a href="{{ route('admin.informasi.dikecualikan.index') }}" class="text-xs font-black text-gray-400 hover:text-[#004a99] uppercase tracking-widest transition-all">
                <i class="fas fa-times mr-2"></i> Batalkan
            </a>
        </div>

        <form action="{{ route('admin.informasi.dikecualikan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
                ...
                <div class="p-8 md:p-12 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            <input type="text" name="jangka_waktu" value="{{ old('jangka_waktu') }}" placeholder="Contoh: 10 Tahun"
                                class="w-full px-8 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 focus:bg-white transition-all font-bold text-[#002b5c]">
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="dasar_hukum" rows="3"
                            class="w-full px-8 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 focus:bg-white transition-all font-bold text-[#002b5c]">{{ old('dasar_hukum') }}</textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-3">
                            ...
                            <textarea name="konsekuensi_dibuka" rows="3"
                                class="w-full px-8 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-green-100 focus:bg-white transition-all font-bold text-gray-700">{{ old('konsekuensi_dibuka') }}</textarea>
                        </div>
                        <div class="space-y-3">
                            ...
                            <textarea name="konsekuensi_ditutup" rows="3"
                                class="w-full px-8 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-red-100 focus:bg-white transition-all font-bold text-gray-700">{{ old('konsekuensi_ditutup') }}</textarea>
                        </div>
                    </div>
