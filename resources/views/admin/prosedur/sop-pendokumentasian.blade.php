...
<div class="min-h-screen bg-[#f8f9fa] p-4 md:p-6 text-gray-800">
    <div class="max-w-7xl mx-auto space-y-8 uppercase">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 font-bold text-gray-800">
            <div>
                ...
                <h1 class="text-3xl font-black text-[#004a99] uppercase tracking-tight">
                    <i class="fas fa-archive mr-2 text-[#ffc107]"></i> SOP Pendokumentasian
                </h1>
                <p class="text-gray-500 font-medium mt-1 uppercase tracking-widest text-[10px]">Manajemen Prosedur Pengelolaan dan Pengarsipan Informasi</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ url('/prosedur/sop-pendokumentasian') }}" target="_blank" class="px-6 py-3 bg-white border border-gray-200 text-[#004a99] font-bold rounded-xl shadow-sm hover:bg-gray-50 transition-all flex items-center">
        ...
        @if(session('success'))
            <div class="bg-blue-100 border-l-4 border-[#004a99] p-4 rounded-xl shadow-sm flex items-center animate-fade-in-down mb-6">
                <div class="w-10 h-10 bg-[#004a99] rounded-full flex items-center justify-center mr-3 shadow-lg shadow-blue-500/20">
                    <i class="fas fa-check text-white"></i>
                </div>
                <p class="text-[#004a99] font-bold text-xs">{{ session('success') }}</p>
            </div>
        @endif

        <form action="{{ route('admin.halaman-custom.store', 'sop_pendokumentasian') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="tagline_hero" value="{{ $settings['sop_pendokumentasian_tagline_hero'] ?? 'Standar Operasional Prosedur Pendokumentasian dan Pengarsipan Informasi Publik' }}"
                                class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-gray-800 font-bold focus:bg-white focus:ring-4 focus:ring-[#004a99]/5 focus:border-[#004a99] outline-none transition-all uppercase">
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 text-gray-800 uppercase font-bold">
                ...
                <div class="bg-white rounded-3xl shadow-xl ring-1 ring-gray-200 overflow-hidden border-t-8 border-[#004a99] group">
                    <div class="p-8 space-y-6">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" onclick="document.getElementById('gambar_sop').click()" class="px-4 py-2 bg-gray-50 text-[9px] font-black text-[#004a99] rounded-lg border border-gray-100 hover:bg-gray-100 transition-all font-bold">UPLOAD BARU</button>
                        </div>
                        ...
                        <div class="relative aspect-[3/4] bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 group-hover:border-[#004a99]/20 transition-all shadow-inner">
                            @if(isset($settings['sop_pendokumentasian_gambar_sop']))
                                <img id="preview_sop" src="{{ asset('storage/halaman/' . $settings['sop_pendokumentasian_gambar_sop']) }}" class="w-full h-full object-contain opacity-90 group-hover:opacity-100 transition-all">
                            @else
                ...
                <div class="bg-white rounded-3xl shadow-xl ring-1 ring-gray-200 overflow-hidden border-t-8 border-[#ffc107] group">
                    <div class="p-8 space-y-6">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xs font-black text-[#004a99] uppercase tracking-[0.2em] flex items-center text-gray-800 uppercase font-bold">
                                <i class="fas fa-project-diagram mr-3 text-[#ffc107]"></i> Infografis Alur Pengarsipan
                            </h3>
                        ...
                        <div class="relative aspect-[3/4] bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 group-hover:border-[#ffc107]/20 transition-all shadow-inner">
                            @if(isset($settings['sop_pendokumentasian_gambar_proses']))
                                <img id="preview_proses" src="{{ asset('storage/halaman/' . $settings['sop_pendokumentasian_gambar_proses']) }}" class="w-full h-full object-contain opacity-90 group-hover:opacity-100 transition-all">
                            @else
                                <div class="w-full h-full flex flex-col items-center justify-center text-gray-300">
                                    <i class="fas fa-images text-5xl mb-3 text-gray-800 opacity-60"></i>
                                    <p class="text-[10px] font-black uppercase tracking-widest text-gray-800 text-gray-800">Alur Belum Tersedia</p>
                                </div>
                            @endif
            ...
            <div class="bg-white h-24 rounded-3xl shadow-2xl ring-1 ring-gray-200 px-8 flex items-center justify-between sticky bottom-6 z-50 animate-fade-in-up mt-12 text-gray-800">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 bg-blue-50 text-[#004a99] rounded-full flex items-center justify-center shadow-sm">
                        <i class="fas fa-archive"></i>
                    </div>
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest text-gray-800">Data Terstruktur & Aman</p>
                </div>
                <div class="flex gap-4">
                    ...
                    <button type="submit" class="px-10 py-3 bg-[#004a99] text-white font-black text-[10px] rounded-xl shadow-lg shadow-blue-500/20 transform hover:scale-105 transition-all tracking-widest text-gray-800">
                        <i class="fas fa-save mr-2 text-[#ffc107]"></i> SIMPAN SOP PENDOKUMENTASIAN
                    </button>
                </div>
            </div>
        </form>
