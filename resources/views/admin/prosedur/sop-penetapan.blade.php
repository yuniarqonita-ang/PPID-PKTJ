...
<div class="space-y-8 animate-fade-in lg:px-8">
            ...
            <div class="space-y-6">
                ...
                <div>
                    <h1 class="text-3xl md:text-5xl font-black tracking-tight leading-tight text-white mb-2">
                        SOP <span class="text-[#ffc107]">Penetapan DIP</span>
                    </h1>
                    <p class="text-blue-50 text-lg font-bold max-w-2xl opacity-90">Prosedur Penetapan Daftar Informasi Publik (DIP) PKTJ.</p>
                </div>
            </div>

            <div class="flex items-center gap-4">
                <a href="http://ppid.pktj.ac.id/prosedur/sop-penetapan-pemutakhiran" target="_blank" class="px-6 py-4 bg-white/10 border border-white/20 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-white/20 transition-all flex items-center">
    ...
    <form action="{{ route('admin.halaman-custom.store', 'sop_penetapan') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
        @csrf
            ...
            <div class="p-10 space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="judul_hero" value="{{ $settings['sop_penetapan_judul_hero'] ?? 'SOP Penetapan Daftar Informasi Publik' }}"
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="tagline_hero" value="{{ $settings['sop_penetapan_tagline_hero'] ?? '' }}"
                            class="w-full px-6 py-5 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 text-lg font-bold text-[#004a99]">
                    </div>
                </div>
            </div>
                ...
                <div class="p-10 space-y-8">
                    ...
                    <div class="relative aspect-[3/4] bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center">
                        @if(isset($settings['sop_penetapan_gambar_sop']))
                            <img id="preview_sop" src="{{ asset('storage/halaman/' . $settings['sop_penetapan_gambar_sop']) }}" class="w-full h-full object-contain">
                        @else
                ...
                <div class="p-10 space-y-8">
                    ...
                    <div class="relative aspect-[3/4] bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center">
                        @if(isset($settings['sop_penetapan_gambar_proses']))
                            <img id="preview_proses" src="{{ asset('storage/halaman/' . $settings['sop_penetapan_gambar_proses']) }}" class="w-full h-full object-contain">
                        @else
                            <div class="text-center">
                                <i class="fas fa-bezier-curve text-6xl text-slate-200 mb-4"></i>
        ...
        <div class="flex justify-end gap-6 pt-10">
            ...
            <button type="submit" class="px-12 py-4 bg-[#004a99] text-white font-black text-sm rounded-xl shadow-2xl hover:bg-black transition-all tracking-widest border-none cursor-pointer uppercase">
                <i class="fas fa-save mr-3 text-[#ffc107]"></i> SIMPAN PROSEDUR PENETAPAN
            </button>
        </div>
    </form>
</div>
