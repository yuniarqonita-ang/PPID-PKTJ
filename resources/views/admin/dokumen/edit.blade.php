...
@php
    ...
    if ($kategori === 'Laporan Layanan') {
        ...
        $title = 'Edit Laporan Layanan Informasi';
        $subtitle = 'Perbarui detail dan file laporan pelayanan informasi publik';
    } elseif ($kategori === 'Laporan Akses') {
        $cancelUrl = route('admin.layanan.laporan-akses');
        $title = 'Edit Laporan Akses Informasi';
        $subtitle = 'Perbarui rekapitulasi data akses informasi publik';
    } elseif ($kategori === 'Laporan Survey') {
        $cancelUrl = route('admin.layanan.laporan-survey');
        $title = 'Edit Laporan Survey Kepuasan';
        $subtitle = 'Perbarui detail data hasil survey kepuasan masyarakat';
    } elseif ($isSop && isset($sopCategoryRoutes[$kategori])) {
        $cancelUrl = $sopCategoryRoutes[$kategori];
        $title = 'Edit Data - ' . $kategori;
        $subtitle = 'Perbarui dokumen dan konfigurasi tampilan halaman SOP';
    } else {
        $cancelUrl = route('admin.dokumen.index');
        $title = 'Edit Dokumen / Laporan';
        $subtitle = 'Ubah data arsip file atau laporan yang ada di sistem';
    }
@endphp
        ...
        <div class="flex items-center justify-between text-gray-800">
            <div class="text-gray-800">
                <h1 class="text-3xl font-black text-[#004a99] uppercase tracking-tight text-gray-800">
                    <i class="fas fa-edit mr-2 text-[#ffc107] text-gray-800"></i> {!! str_replace('Laporan', '<span class="text-gray-800">Laporan</span>', $title) !!}
                </h1>
        ...
        <form action="{{ route('admin.dokumen.update', $dokumen->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl ring-1 ring-gray-200 overflow-hidden">
                <div class="p-8 md:p-12 space-y-8">
                    ...
                    <!-- Title Field -->
                    <div class="space-y-3">
                        <label class="text-xs font-black text-[#004a99] uppercase tracking-[2px] block text-gray-800">Judul Dokumen / Laporan</label>
                        <input type="text" name="judul" required value="{{ old('judul', $dokumen->judul) }}"
                    ...
                    @if(in_array($kategori, ['Laporan Layanan', 'Laporan Akses', 'Laporan Survey']))
                    ...
                    @else
                        <div class="space-y-3">
                            ...
                            <select name="kategori" required
                                ...
                                <option value="Umum" {{ old('kategori', $dokumen->kategori) == 'Umum' ? 'selected' : '' }}>Umum / Lainnya</option>
                                <option value="Laporan Layanan" {{ old('kategori', $dokumen->kategori) == 'Laporan Layanan' ? 'selected' : '' }}>Laporan Layanan Informasi</option>
                                <option value="Laporan Akses" {{ old('kategori', $dokumen->kategori) == 'Laporan Akses' ? 'selected' : '' }}>Laporan Akses Informasi</option>
                                <option value="Laporan Survey" {{ old('kategori', $dokumen->kategori) == 'Laporan Survey' ? 'selected' : '' }}>Laporan Survey Kepuasan</option>
                                <option value="Regulasi" {{ old('kategori', $dokumen->kategori) == 'Regulasi' ? 'selected' : '' }}>Regulasi / Aturan</option>
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="date" name="tanggal" required value="{{ old('tanggal', $dokumen->tanggal ? \Carbon\Carbon::parse($dokumen->tanggal)->format('Y-m-d') : date('Y-m-d')) }}"
                    ...
                    <div class="space-y-3 text-gray-800">
                        ...
                        <div class="rounded-3xl overflow-hidden border-2 border-slate-100 text-gray-800">
                            <textarea name="deskripsi" id="editor" class="tinymce-editor text-gray-800">{{ old('deskripsi', $dokumen->deskripsi) }}</textarea>
                        </div>
                        @error('deskripsi') <p class="text-red-500 text-xs mt-1 font-bold">{{ $message }}</p> @enderror
                    </div>

                    @php
                        $isGDrive = str_starts_with($dokumen->file_path ?? '', 'http://') || str_starts_with($dokumen->file_path ?? '', 'https://');
                    @endphp
                    ...
                    <div class="p-6 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                        ...
                        @if($isGDrive)
                            <div class="flex items-center text-blue-600 font-bold text-sm">
                                <i class="fab fa-google-drive mr-2 text-lg"></i> Link Google Drive: 
                                <a href="{{ $dokumen->file_path }}" target="_blank" class="ml-2 underline hover:text-blue-800 truncate max-w-md">{{ $dokumen->file_path }}</a>
                            </div>
                        @elseif($dokumen->file_path)
                            <div class="flex items-center text-emerald-600 font-bold text-sm">
                                <i class="fas fa-file-alt mr-2 text-lg"></i> File Lokal: 
                                <span class="ml-2 text-slate-700 font-semibold truncate">{{ $dokumen->file_name ?? basename($dokumen->file_path) }}</span>
                                <span class="text-xs font-medium text-slate-400 ml-2">({{ $dokumen->file_size ?: 'Ukuran tidak diketahui' }})</span>
                            </div>
                        @else
                    ...
                    <!-- File Upload -->
                    <div class="space-y-3 text-gray-800">
                        <label class="text-xs font-black text-[#004a99] uppercase tracking-[2px] block text-gray-800">Ganti Lampiran Dokumen (Biarkan kosong jika tidak diubah)</label>
                        <div class="relative group">
                            ...
                            <div onclick="document.getElementById('file').click()" 
                                ...
                                <p id="file-name-display" class="text-sm font-black text-slate-400 uppercase tracking-widest text-center">Pilih file baru untuk mengganti</p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-3 text-gray-800">
                        <label class="text-xs font-black text-[#004a99] uppercase tracking-[2px] flex items-center">
                            <i class="fab fa-google-drive mr-2 text-blue-500"></i> ATAU Ganti dengan Link Google Drive
                        </label>
                        <input type="url" name="gdrive_link" value="{{ old('gdrive_link', $isGDrive ? $dokumen->file_path : '') }}"
                        ...
                        <p class="text-[10px] text-blue-500 font-bold mt-1">
                            <i class="fas fa-info-circle mr-1"></i> Jika diisi, link ini menggantikan file lokal saat ini.
                        </p>
                    ...
                    <div class="pt-6 border-t border-slate-50 flex items-center justify-between text-gray-800">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="aktif" value="1" {{ $dokumen->aktif ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div class="pt-6 border-t border-slate-50 flex items-center justify-between p-6 bg-gradient-to-r from-blue-50 to-white rounded-3xl border border-blue-100 shadow-sm">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_blurred" value="1" {{ $dokumen->is_blurred ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div class="pt-6 border-t border-slate-50 flex items-center justify-between p-6 bg-gradient-to-r from-emerald-50 to-white rounded-3xl border border-emerald-100 shadow-sm">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="bisa_download" value="1" {{ $dokumen->bisa_download ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="flex justify-end gap-4 pt-4 text-gray-800">
                <button type="submit" class="px-16 py-6 bg-[#004a99] text-white font-black text-xs uppercase tracking-[3px] rounded-[2rem] shadow-2xl shadow-blue-900/20 hover:bg-black hover:-translate-y-1 transition-all border-none cursor-pointer">
                    <i class="fas fa-check-circle mr-3 text-[#ffc107]"></i> Perbarui Dokumen
                </button>
            </div>
