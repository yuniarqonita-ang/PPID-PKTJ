...
<div class="space-y-8 animate-fade-in lg:px-8">
    ...
    <div class="bg-gradient-to-br from-[#004a99] via-[#005bb5] to-[#006ccf] rounded-[2rem] p-10 md:p-12 shadow-xl text-white relative overflow-hidden mb-10">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div class="space-y-6">
                ...
                <div>
                    <h1 class="text-3xl md:text-5xl font-black tracking-tight leading-tight text-white mb-2">
                        SOP <span class="text-[#ffc107]">Keberatan Informasi</span>
                    </h1>
            ...
            <div class="flex items-center gap-4">
                <a href="http://ppid.pktj.ac.id/prosedur/sop-pengajuan-keberatan" target="_blank" class="px-6 py-4 bg-white/10 border border-white/20 text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-white/20 transition-all flex items-center">
    ...
    <form id="sop-form" action="{{ route('admin.halaman-custom.store', 'sop_keberatan') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
        @csrf
            ...
            <div class="p-10 space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="judul_hero" value="{{ $settings['sop_keberatan_judul_hero'] ?? 'SOP Pengajuan Keberatan Informasi' }}"
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="tagline_hero" value="{{ $settings['sop_keberatan_tagline_hero'] ?? '' }}"
                            class="w-full px-6 py-5 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 text-lg font-bold text-[#004a99]">
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div class="p-10 space-y-8">
                    ...
                    <div class="relative aspect-[3/4] bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center">
                        @if(isset($settings['sop_keberatan_gambar_sop']))
                            <img id="preview_sop" src="{{ asset('storage/halaman/' . $settings['sop_keberatan_gambar_sop']) }}" class="w-full h-full object-contain">
                        @else
                ...
                <div class="p-10 space-y-8">
                    ...
                    <div class="relative aspect-[3/4] bg-slate-50 rounded-[2rem] overflow-hidden border-2 border-dashed border-slate-200 flex items-center justify-center">
                        @if(isset($settings['sop_keberatan_gambar_proses']))
                            <img id="preview_proses" src="{{ asset('storage/halaman/' . $settings['sop_keberatan_gambar_proses']) }}" class="w-full h-full object-contain">
                        @else
                            <div class="text-center">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl shadow-xl border-2 border-slate-100 p-10">
                ...
                <div class="space-y-4">
                    ...
                    <textarea name="konten" class="tinymce-editor">{{ $settings['sop_keberatan_konten'] ?? '' }}</textarea>
