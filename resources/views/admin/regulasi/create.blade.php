...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
            ...
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Tambah Regulasi / Peraturan</h1>
            <p class="text-sm text-slate-500 font-medium mt-0.5">Tambahkan dasar hukum keterbukaan informasi publik resmi.</p>
        </div>
    </div>

    <form action="{{ route('admin.regulasi.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 space-y-2">
                ...
                <input type="text" name="judul" value="{{ old('judul') }}" required placeholder="Contoh: Undang-Undang Nomor 14 Tahun 2008" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-[#004a99] font-medium text-sm transition-all">
            ...
            <div class="space-y-2">
                ...
                <input type="number" name="tahun" value="{{ old('tahun', date('Y')) }}" placeholder="2025" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-[#004a99] font-medium text-sm transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="nomor" value="{{ old('nomor') }}" placeholder="Contoh: UU 14/2008 atau PM 46/2018" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-[#004a99] font-medium text-sm transition-all">
            </div>
        </div>

        <div class="space-y-2">
            <label class="block text-xs font-black text-slate-700 uppercase tracking-wider">Deskripsi / Perihal</label>
            <textarea name="deskripsi" rows="3" placeholder="Contoh: tentang Keterbukaan Informasi Publik (KIP)" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-[#004a99] font-medium text-sm transition-all">{{ old('deskripsi') }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5 bg-slate-50 rounded-2xl border border-slate-200">
            <div class="space-y-2">
                <label class="block text-xs font-black text-slate-700 uppercase tracking-wider">Upload Dokumen PDF (Lokal)</label>
                ...
                <p class="text-[11px] text-slate-400">Maksimal ukuran file: 20MB (.pdf, .docx).</p>
            </div>

            <div class="space-y-2">
                <label class="block text-xs font-black text-slate-700 uppercase tracking-wider">ATAU Tautan Eksternal (JDIH / Drive)</label>
                <input type="url" name="link_download" value="{{ old('link_download') }}" placeholder="https://jdih.dephub.go.id/..." class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#004a99] font-medium text-xs transition-all">
        ...
        <div class="flex items-center justify-between pt-4 border-t border-slate-100">
            <label class="flex items-center gap-2.5 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-5 h-5 text-[#004a99] rounded border-slate-300 focus:ring-blue-500">
                <span class="text-sm font-bold text-slate-700">Publikasikan Regulasi Sekarang</span>
            </label>
