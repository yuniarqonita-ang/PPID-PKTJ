...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-slate-400 mt-1">Kelola laporan layanan informasi publik yang tersedia</p>
        </div>
            ...
            <div class="mb-12 space-y-8 bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-2xl ring-1 ring-white/10 relative overflow-hidden p-8 rounded-2xl border-2 border-blue-100 shadow-sm">
                <div class="flex items-center space-x-3 mb-2">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400">Sesuaikan tampilan awal halaman Laporan Layanan di publik</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="group">
                        ...
                        <input type="text" name="judul_hero" 
                               ...
                               value="{{ $settings['laporan_layanan_judul_hero'] ?? 'Laporan Layanan Informasi Publik' }}">
                    </div>
                    <div class="group">
                        <label class="block text-sm font-bold text-slate-200 mb-2">Tagline Hero</label>
                        <input type="text" name="tagline_hero" 
                               class="w-full px-4 py-3 rounded-xl border-2 border-slate-600/30 focus:border-cyan-500 focus:ring-cyan-500 focus:ring-4 focus:ring-blue-500/10 transition-all bg-slate-900/60 border-slate-600/50 text-white placeholder-slate-400 shadow-inner focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-400"
                               value="{{ $settings['laporan_layanan_tagline_hero'] ?? 'Laporan kinerja PPID dalam melayani permintaan informasi publik' }}">
                    </div>
                </div>
            </div>
                        ...
                        <div class="group">
                            ...
                            <select name="tahun_laporan" id="tahun_laporan" 
                                ...
                                @for($year = date('Y'); $year >= date('Y') - 5; $year--)
                                    <option value="{{ $year }}" {{ isset($settings['laporan_layanan_tahun_laporan']) && $settings['laporan_layanan_tahun_laporan'] == $year ? 'selected' : '' }}>{{ $year }}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="group">
                                ...
                                <input type="date" name="periode_mulai" id="periode_mulai" 
                                       ...
                                       value="{{ $settings['laporan_layanan_periode_mulai'] ?? '' }}" required>
                            </div>
                            <div class="group">
                                <label class="block text-lg font-bold text-slate-200 mb-3 flex items-center">
                                ...
                                <input type="date" name="periode_selesai" id="periode_selesai" 
                                       ...
                                       value="{{ $settings['laporan_layanan_periode_selesai'] ?? '' }}" required>
                            </div>
                        </div>
                    </div>
...
{!! $settings['laporan_layanan_ringkasan_eksekutif'] ?? 'Tuliskan ringkasan eksekutif laporan layanan informasi publik...' !!}
                                </textarea>
                            </div>
...
{!! $settings['laporan_layanan_isi_laporan'] ?? 'Tuliskan isi laporan layanan informasi publik secara lengkap dan detail...' !!}
                                </textarea>
                            </div>
                            <p class="text-sm text-slate-400 mt-2">
                                <i class="fas fa-info-circle mr-1"></i>
                        ...
                        <div class="group">
                            ...
                            @if(isset($settings['laporan_layanan_file_laporan']) && $settings['laporan_layanan_file_laporan'])
                                <div class="mt-3">
                                    ...
                                    <a href="{{ asset('storage/halaman/' . $settings['laporan_layanan_file_laporan']) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-slate-100 border border-slate-600/30 rounded-lg text-slate-200 hover:bg-slate-200 transition">
                    ...
                    <div class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-900/20 to-indigo-900/20 border border-blue-600/30 p-6 shadow-lg">
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center mb-3">
                                ...
                                <h3 class="text-lg font-bold text-blue-300">Tentang Laporan Layanan</h3>
                            </div>
                            <p class="text-sm text-blue-400 leading-relaxed">
                                Laporan Layanan Informasi Publik adalah dokumen resmi yang memuat informasi statistik dan evaluasi tentang layanan informasi yang disediakan kepada masyarakat sesuai peraturan perundang-undangan.
                            </p>
                        </div>
                    </div>
