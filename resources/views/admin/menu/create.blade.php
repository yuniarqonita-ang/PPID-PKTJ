...
<div class="space-y-8 animate-fade-in lg:px-8">
    <div>
        <h1 class="text-3xl font-black text-[#004a99] tracking-tight uppercase">Tambah Menu Baru</h1>
        <p class="text-slate-500 text-sm font-semibold mt-1">Buat menu navigasi baru untuk website PPID PKTJ.</p>
    </div>
                ...
                <div class="bg-white rounded-[2rem] shadow-xl border border-slate-200 p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="nama" value="{{ old('nama') }}" required class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 focus:bg-white transition-all font-bold text-lg text-[#002b5c]">
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="url" value="{{ old('url') }}" class="w-full px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-[#004a99]/10 focus:bg-white transition-all font-bold text-[#002b5c]" placeholder="Contoh: https://dephub.go.id atau /layanan-informasi">
                        ...
                        <div class="bg-slate-50 p-6 rounded-2xl space-y-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <label class="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200/60 cursor-pointer select-none">
                                    <input type="checkbox" name="is_editor" value="1" {{ old('is_editor', 1) ? 'checked' : '' }} class="w-5 h-5 text-[#004a99] rounded border-slate-300">
                                    <span class="text-xs font-bold text-slate-700">Teks Editor</span>
                                </label>
                                <label class="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200/60 cursor-pointer select-none">
                                    <input type="checkbox" name="is_table" value="1" {{ old('is_table') ? 'checked' : '' }} class="w-5 h-5 text-[#004a99] rounded border-slate-300">
                                    <span class="text-xs font-bold text-slate-700">Tabel Data</span>
                                </label>
                                <label class="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200/60 cursor-pointer select-none">
                                    <input type="checkbox" name="is_chart" value="1" {{ old('is_chart') ? 'checked' : '' }} class="w-5 h-5 text-[#004a99] rounded border-slate-300">
                                    <span class="text-xs font-bold text-slate-700">Bagan/Gambar</span>
                                </label>
                                <label class="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200/60 cursor-pointer select-none">
                                    <input type="checkbox" name="is_form" value="1" {{ old('is_form') ? 'checked' : '' }} class="w-5 h-5 text-[#004a99] rounded border-slate-300">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="rounded-3xl overflow-hidden border-2 border-slate-100">
                                <textarea name="konten" id="editor_konten" class="tinymce-editor">{{ old('konten') }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="relative z-10 space-y-6">
                        <div class="flex items-center gap-3 border-b border-white/10 pb-6">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-blue-200/60 font-bold uppercase mt-1">Simpan menu baru</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div class="space-y-2">
                                ...
                                <select name="penempatan" class="w-full px-4 py-3 bg-white/10 border-2 border-white/10 rounded-xl focus:ring-4 focus:ring-[#ffc107]/20 focus:bg-slate-800/90 transition-all font-bold text-white text-sm">
                                    <option value="header" class="text-slate-800" {{ old('penempatan', 'header') === 'header' ? 'selected' : '' }}>Header (Navigasi Atas)</option>
                                    <option value="footer" class="text-slate-800" {{ old('penempatan') === 'footer' ? 'selected' : '' }}>Footer (Navigasi Bawah)</option>
                                    <option value="both" class="text-slate-800" {{ old('penempatan') === 'both' ? 'selected' : '' }}>Keduanya (Header & Footer)</option>
                                </select>
                            </div>

                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-blue-200/60 uppercase tracking-[2px] block">Urutan Menu</label>
                                <input type="number" name="urutan" value="{{ old('urutan', 0) }}" class="w-full px-4 py-3 bg-white/10 border-2 border-white/10 rounded-xl focus:ring-4 focus:ring-[#ffc107]/20 focus:bg-white/20 transition-all font-bold text-white text-sm">
                            </div>

                            <label class="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/10 cursor-pointer select-none">
                                <input type="checkbox" name="aktif" value="1" {{ old('aktif', 1) ? 'checked' : '' }} class="w-5 h-5 text-[#ffc107] rounded border-white/20 bg-transparent">
