                    ...
                    <form action="{{ route('admin.kontak.update', 1) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="email" name="email" value="{{ old('email') }}" class="w-full px-4 py-3 border-2 border-slate-600/50 text-white placeholder-slate-500 rounded-xl focus:border-cyan-400 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none transition-all duration-300 shadow-inner bg-slate-900/60 border-slate-600/50 text-white placeholder-slate-400 shadow-inner focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-400" required>
                            </div>
                            <div>
                                <label class="block text-slate-300 font-bold mb-2">Telepon</label>
                                <input type="tel" name="telepon" value="{{ old('telepon') }}" class="w-full px-4 py-3 border-2 border-slate-600/50 text-white placeholder-slate-500 rounded-xl focus:border-cyan-400 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none transition-all duration-300 shadow-inner bg-slate-900/60 border-slate-600/50 text-white placeholder-slate-400 shadow-inner focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-400" required>
                            </div>
                        </div>
