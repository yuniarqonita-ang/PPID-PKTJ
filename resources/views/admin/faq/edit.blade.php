        ...
        <form action="{{ route('admin.faq.update', $faq) }}" method="POST" class="bg-white rounded-lg shadow-md p-8">
            ...
            <div class="mb-6">
                ...
                <textarea name="jawaban" id="jawaban" rows="8" placeholder="Masukkan jawaban FAQ" required
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('jawaban') border-red-500 @enderror">{{ old('jawaban', $faq->jawaban) }}</textarea>
                @error('jawaban')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
